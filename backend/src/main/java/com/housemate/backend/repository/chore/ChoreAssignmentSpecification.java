...
public class ChoreAssignmentSpecification {
    ...
    public static Specification<ChoreAssignment> buildAssignmentFilter(UUID householdId,
        ...
        return (root, query, criteriaBuilder) -> {
            ...
            predicates.add(criteriaBuilder.equal(root.get("chore").get("household").get("id"), householdId));
            ...
            if (filter.descriptionContains() != null && !filter.descriptionContains().isEmpty()) {
                predicates.add(criteriaBuilder.like(
                        criteriaBuilder.lower(root.get("chore").get("description")),
            ...
            if (filter.dateRange() != null) {
                if (filter.dateRange().startDate() != null) {
                    predicates.add(criteriaBuilder.greaterThanOrEqualTo(root.get("chore").get("dueDate"),
                                                                        filter.dateRange().startDate()));
                }
                if (filter.dateRange().endDate() != null) {
                    predicates.add(criteriaBuilder.lessThanOrEqualTo(root.get("chore").get("dueDate"),
