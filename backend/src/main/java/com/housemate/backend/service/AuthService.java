...
public class AuthService {
	...
	public LoginResponseDTO login(@NonNull LoginRequestDTO dto) throws IllegalArgumentException, AuthenticationException {
		...
		Assert.isTrue(isValidEmail(dto.email()), "Email in LoginRequestDTO must be a valid email address");
	...
	public LoginResponseDTO register(@NonNull RegisterRequestDTO dto) throws IllegalArgumentException {
		...
		Assert.isTrue(isValidEmail(dto.email()), "Email in RegisterRequestDTO must be a valid email address");
        ...
        Assert.isTrue(
            dto.iban() == null || dto.iban().matches("^[A-Z]{2}\\d{2}[A-Z0-9]{1,30}$"),
